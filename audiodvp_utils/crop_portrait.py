...
def calc_bbox(image_list, gpu_ids, batch_size=5):
    """Batch infer of face location, batch_size should be factor of total frame number."""
    fa = face_detection.FaceAlignment(face_detection.LandmarksType._2D, flip_input=False, device='cuda:{}'.format(gpu_ids))
...
def crop_image(data_dir, dest_size, crop_level, vertical_adjust, gpu_ids):
    image_list = util.get_file_list(os.path.join(data_dir, 'full'))
    H, W, _ = face_recognition.load_image_file(image_list[0]).shape
    top, right, bottom, left = calc_bbox(image_list, gpu_ids)
    height = bottom - top
    width = right - left
...
def crop_per_image(data_dir, dest_size, crop_level, gpu_ids):
    fa = face_detection.FaceAlignment(face_detection.LandmarksType._2D, flip_input=False, device='cuda:{}'.format(gpu_ids))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dest_size', type=int, default=144)
    parser.add_argument('--crop_level', type=float, default=1.2, help='Adjust crop image size.')
    ...
    crop_image(args.data_dir, dest_size=args.dest_size, crop_level=args.crop_level, vertical_adjust=args.vertical_adjust, gpu_ids=args.gpu_ids)
