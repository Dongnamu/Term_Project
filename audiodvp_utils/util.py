...
def extract_face_emb(image_list, save_path, transforms_input, gpu_id):
    facenet = InceptionResnetV1(pretrained='vggface2').eval().to('cuda:{}'.format(gpu_id))
    ...
    for i in range(len(image_list)):
        ...
        input = transforms_input(image).to('cuda:{}'.format(gpu_id))
        input = input.reshape(1, 3, 224, 224)
        face_emb = facenet(input)

        face_emb_dict[os.path.basename(image_name)] = face_emb.squeeze().detach().to('cpu')
