...
def get_reference_dict(data_dir):
    image = cv2.imread(os.path.join(data_dir, 'reference_frame.jpg'))
...
def multiProcess(im, data_dir, reference_dict):
    ...
    min_tracking_confidence=0.5) as face_mesh:
        ...
        if results.multi_face_landmarks is not None:
            ...
            f = open(os.path.join(data_dir, 'failed.txt'),'w')
            f.close()

def pose_normalization():
    data_dir = args.data_dir
    videos = [os.path.join(data_dir, x) for x in natsorted(os.listdir(data_dir)) if os.path.isdir(os.path.join(data_dir, x))]
    for video in videos:
        create_dirs(video)
    image_list = [util.get_file_list(os.path.join(video, 'crop')) for video in videos]
    ...
    for il in image_list:
        ref_dict = [reference_dict for _ in range(len(il))]
        reference_dicts.append(ref_dict)

    pool = Pool(processes=40)
    for i, video in tqdm(enumerate(videos)):
        data_dirs = [video for _ in range(len(reference_dicts[i]))]
        pool.starmap(multiProcess, zip(image_list[i], data_dirs, reference_dicts[i]))
    pool.terminate()
    pool.join()
...
def create_dirs(path):
    os.makedirs(os.path.join(path, 'mesh_dict'), exist_ok=True)
    if args.draw_mesh:
        os.makedirs(os.path.join(path, 'mesh_image'), exist_ok=True)
    
    if args.draw_norm_mesh:
        os.makedirs(os.path.join(path, 'mesh_norm_image'), exist_ok=True)

    if args.draw_norm_3d_mesh:
        os.makedirs(os.path.join(path, 'mesh_norm_3d_image'), exist_ok=True)
